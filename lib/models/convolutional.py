...
class ConvolutionalDLVM(LatentVariableModel):
    """
    A generic convolutional dynamical latent variable model.

    Args:
        model_config (dict): dictionary containing model configuration params
    """
    def __init__(self, model_config):
        super(ConvolutionalDLVM, self).__init__(model_config)
        self._construct(model_config)

    def _construct(self, model_config):
        """
        Method for constructing the model using the model configuration file.
    ...
    #         self.mean_output = ConvolutionalLayer(model_config['n_filters_dec'][0], 3, model_config['filter_size'][0], non_linearity='sigmoid', weight_norm=model_config['weight_norm_dec'])
    ...
    #         else:
    #             self.log_var_output = ConvolutionalLayer(model_config['n_filters_dec'][0], 3, model_config['filter_size'][0], weight_norm=model_config['weight_norm_dec'])
    #
